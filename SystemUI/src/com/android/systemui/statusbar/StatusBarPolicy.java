...
public class StatusBarPolicy {
	...
	Tema sTema;
	/**
	 * edge=0 gprs=1 hdspa=2 3g=3
	 * 
	 */
	public static String[] TYPE = { "EDGE", "GPRS", "HSDPA", "3G" };
	String SINYALTYPE = TYPE[0];
	int pos = 0;

	private static final String TAG = "StatusBarPolicy";
	StatusBarService sbService;
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
				updateBattery(intent);
					...
					|| action.equals(Model.UPDATE_STATUSBAR)) {
				sbService.setJam();
	...
	public StatusBarPolicy(Context context) {
		sbService = (StatusBarService) context;
	...
	private PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
		...
		public void onDataConnectionStateChanged(int state, int networkType) {
			...
			updateDataIcon();
			sbService.setTraffic(hasService());
		}
	...
	private final void updateSignalStrength() {
		...
		if (mServiceState == null
			...
			sbService.setSinyalStrenght(-1);
			sbService.setTraffic(false);
			mService.setIcon("phone_signal", mPhoneSignalIconId, 0);
			return;
		}
		...
		sbService.setSinyalStrenght(iconLevel);
		mService.setIcon("phone_signal", mPhoneSignalIconId, 0);
	}
		...
		if (!isCdma()) {
			...
			if (mSimState == IccCard.State.READY
				...
				if (hasService()
					...
					sbService.setSinyalIO(SINYALTYPE, pos);
				} else {
					visible = false;
					sbService.setSinyalIO(SINYALTYPE, -666);
					sbService.setTraffic(false);
	...
	private final void updateVolume() {
		...
		sbService.setRingIcon(ringerMode);
	...
	private final void updateBluetooth(Intent intent) {
		...
		mService.setIconVisibility("bluetooth", mBluetoothEnabled);
		sbService.showblutut(mBluetoothEnabled);
	}
